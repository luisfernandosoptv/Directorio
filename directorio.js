...
function pintar() {
    if (amigos.length > 0) {
        ...
        amigos.forEach((contacto) => {
            ...
            amigo.innerHTML = `<p>${contacto.nombre}</p><button class="muestraDetalles"><input type="hidden" value="${contacto.telefono}">Detalles</button> <button class="eliminarElemento"><input type="hidden" value="${contacto.telefono}">Eliminar</button>`;
...
function eliminaElementos(tel){
    let eliminarElementos=document.getElementsByClassName("eliminarElemento");
    eliminarElementos.addEventListener("onclick",elimina=>{
        amigos.splice(a);
    });
}